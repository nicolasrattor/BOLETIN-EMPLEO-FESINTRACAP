---
title: "BOLETÍN TRIMESTRE NOVIEMBRE-ENERO 2020-2021"
author: "OBSERVATORIO CALIDAD DEL EMPLEO TRABAJADORAS DE CASA PARTICULAR"
date: "Chile. Edición nº 7"
output:
  pdf_document:
...
> En un año las trabajadoras de casa particular ocupadas han disminuido en más de la tercera parte por efecto del COVID-19 (`r round((((round(as.numeric(mam[nrow(mam),12])*1000))-(round(as.numeric(mam[nrow(mam)-12,12])*1000)))/(round(as.numeric(mam[nrow(mam)-12,12])*1000)))*100,1)`%). Si a finales de 2019 estas eran `r round(as.numeric(mam[nrow(mam)-12,12])*1000)` a lo largo de Chile, actualmente estas llegan a solamente a `r round(as.numeric(mam[nrow(mam),12])*1000)`. Respecto al trimestre anterior, se observa una leve recuperación en la ocupación (`r round((((round(as.numeric(mam[nrow(mam),12])*1000))-(round(as.numeric(mam[nrow(mam)-1,12])*1000)))/(round(as.numeric(mam[nrow(mam)-1,12])*1000)))*100,1)`%). 

> La recuperación de la ocupación en el sector es clara desde junio, pero la mayor parte de esta recuperación se explica por la recuperación del empleo informal. En el sector informal, desde junio hasta la fecha se crearon 23.689 puestos de trabajo, mientras que en el formal solamente se han creado 7.185. De cada 100 nuevos puestos de trabajo, solamente 23 son formales. 
...
g1<-mam %>%
  ...
  geom_text_repel(aes(label=ifelse(trimestre=="Nov - Ene", format(round(trabajadores), big.mark = ".", scientific = FALSE),"")),
            vjust=-3,colour="black")

ggsave(plot = g1,
  filename = "../Output/Gráficos/Gráfico1_NDE.png",
  device = "png",
  dpi = "retina",
  units = "cm",
  width = 35,
...
![Número de trabajadoras de casa particular ocupadas desde 2010 a 2021. Total, puertas adentro y puertas afuera](../Output/Gráficos/Gráfico1_NDE.png)
...
g2<-inf %>%
  ...
  labs(x="Tiempo",
       y = "Trabajadoras",
       caption = "Elaboración propia en base a Encuesta Nacional de Empleo del INE") +
  geom_text_repel(aes(label=ifelse(trimestre%in%c("Nov - Ene","Mar - May"), format(round(trabajadores), big.mark = ".", scientific = FALSE),"")),
                  vjust=-3,colour="black")

ggsave(plot = g2,
  filename = "../Output/Gráficos/Gráfico2_NDE.png",
  device = "png",
  dpi = "retina",
  units = "cm",
  width = 35,
