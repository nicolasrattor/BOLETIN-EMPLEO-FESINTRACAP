...
ene2020_10 %>% select(fact_cal) %>% sum()

## Informalidad ENE: 76.233. Formales: 108.201
ene2020_10 %>% group_by(ocup_form) %>% summarise(sum(fact_cal))
...
for (i in 1:nrow(base)){
  base[i,2]<-dfs[[i]] %>% as.data.frame() %>% select(fact_cal) %>% sum()
}
...
## Exportar base
library(writexl)
write_xlsx(base,"Output/informalidad.xlsx", col_names = TRUE,format_headers = TRUE)
